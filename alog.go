...
func (l Logger) Panic(msg string, args ...any) error {
	...
	_, err := preparedMsg.WriteTo(l.writer)
	if err != nil {
		panicMsg = fmt.Sprintf("error during panic: %v (origin reason: %s)", err, msg)
	}
...
func (l Logger) prepareMsg(s string, lvl Level, args ...any) *strings.Reader {
	...
	msg := fmt.Sprintf("%s | %s | %s", time.Now().In(l.timezone).Format(time.RFC3339), levels[lvl], formattedMsg)
